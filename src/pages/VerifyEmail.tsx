...
const VerifyEmail = () => {
  ...
  const handleChange = (index: number, value: string) => {
    ...
    if (upperValue.length > 1) {
      ...
      if (upperValue && index < CODE_LENGTH - 1) {
  ...
  const handleVerify = async () => {
    ...
    if (otp.length !== CODE_LENGTH) {
      toast({
        title: "Invalid code",
        description: `Please enter the full ${CODE_LENGTH}-digit verification code.`,
        variant: "destructive",
      });
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error?.message || "Something went wrong while resending the verification code.",
        variant: "destructive",
      });
    } finally {
      setResending(false);
  ...
  useEffect(() => {
    const filled = code.every((char) => char !== "");
          ...
          <div className="flex justify-center gap-2" onPaste={handlePaste}>
            {code.map((char, index) => (
              <motion.input
                ...
                className={`w-12 h-14 sm:w-14 sm:h-16 text-center text-lg sm:text-xl font-bold rounded-xl border-2 bg-background text-foreground focus:outline-none transition-all duration-200 uppercase ${
                  char
