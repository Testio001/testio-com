...
const Auth = () => {
  ...
  const handleVerifyResetCode = async () => {
    if (!resetCode || resetCode.length < RESET_CODE_LENGTH) {
      toast({
        title: "Error",
        description: `Enter the ${RESET_CODE_LENGTH}-digit code from your email`,
        variant: "destructive",
      });
      return;
    }
              ...
              {resetStep === "request" ? (
                <>
                  <p className="text-sm text-muted-foreground">
                    Enter your email and we'll send you an {RESET_CODE_LENGTH}-digit code:
                  </p>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                    <input
                ...
                <>
                  <p className="text-sm text-muted-foreground">
                    Enter the {RESET_CODE_LENGTH}-digit code sent to{" "}
                    <span className="text-foreground font-medium">{forgotEmail}</span> and choose a new password.
                  </p>
                  <input
                    type="text"
                    inputMode="numeric"
