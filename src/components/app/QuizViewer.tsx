  ...
  if (showResult) {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-lg mx-auto text-center py-10">
        <div className={`text-6xl font-black mb-4 ${pct >= 70 ? "text-turbo-green" : "text-destructive"}`}>
          {pct}%
        </div>
        ...
        <button onClick={restart} className="btn-turbo-primary text-sm !py-2 !px-6 inline-flex items-center gap-2">
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <motion.div key={currentIndex} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <button
                ...
                className={`w-full text-left p-4 rounded-xl border transition-all ${
                  showFeedback && isCorrect
                    ? "border-turbo-green bg-turbo-green/10"
                    : showFeedback && isSelected && !isCorrect
                    ? "border-destructive bg-destructive/10"
                    : isSelected
                    ? "border-primary bg-primary/10"
                    : "border-border hover:border-primary/30 bg-turbo-card"
                }`}
              >
                <div className="flex items-center gap-3">
                  ...
                  {showFeedback && isCorrect && <CheckCircle2 className="w-5 h-5 text-turbo-green" />}
        ...
        {selectedAnswer !== null && (
          <button onClick={nextQuestion} className="btn-turbo-primary text-sm !py-2 !px-6 mt-6">
